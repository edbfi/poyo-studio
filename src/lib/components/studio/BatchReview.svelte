  ...
  >
    ...
    <div class="flex min-h-0 flex-1 flex-col">
      <div class="border-b border-border px-5 py-3 text-xs leading-5 text-muted-foreground">
        <p>
          Local batch · sequential submission · each item remains an independent recoverable job.
        </p>
        <p class="mt-1">
          Each submitted item is a separate billed Poyo job. Exact credits appear only after
          completion.
        </p>
        <p class="mt-1">{settledCount} complete · {draftCount} ready to submit</p>
        <p class="mt-1">
          Estimated ready batch:
          {readyEstimate.credits === null ? 'unavailable' : `${readyEstimate.credits} credits`}
  ...
  : `${settledCharges.credits} credits · ${settledCharges.actionCount} settled Poyo task${settledCharges.actionCount === 1 ? '' : 's'}`}
              ...
              <li class="rounded-[var(--radius)] border border-border bg-card p-3">
                ...
                {:else if item.estimate?.availability === 'available'}
                  <p class="mt-1 text-xs text-muted-foreground">
                    Estimated credits: {item.estimate.credits} · {item.estimate.provenance} ·
                    {item.estimate.freshness}
                  </p>
                {:else}
                  ...
                  <div class="mt-3 flex gap-2 overflow-x-auto">
                    {#each item.outputs.filter((output) => output.mediaUrl) as output (output.outputId)}
                      <a
                        ...
                        target="_blank"
                        rel="noopener"
                        class="focus-ring block size-20 shrink-0 overflow-hidden rounded border border-border bg-stage"
                        aria-label={`Open result for batch item ${index + 1}`}
                      >
                        ...
                        {:else}
                          <img
                            src={output.mediaUrl ?? ''}
                            alt={`Generated result for batch item ${index + 1}`}
                            class="size-full object-contain"
                          >
                        {/if}
                      </a>
                ...
                {/if}
                {#if item.error}
                  <p class="mt-2 text-xs leading-5 text-destructive">{item.error}</p>
                {/if}
                <div class="mt-3 flex flex-wrap gap-1.5">
                  ...
                  {#if item.state === 'unknown'}
                    <Button size="sm" variant="outline" onclick={() => onreconcile(item)}
                      >Check action</Button
                    >
                    <Button size="sm" variant="ghost" onclick={() => onabandon(item)}
                  ...
                  {/if}
                  {#if item.job}
                    <LinkButton href={`/jobs?selected=${item.job.id}`} variant="ghost"
                      >View job</LinkButton
                    >
                  {/if}
        ...
        {:else}
          <div class="rounded-[var(--radius)] bg-muted px-4 py-8 text-center">
            ...
            <p class="mt-1 text-xs leading-5 text-muted-foreground">
              Close this panel, prepare a valid setup, then add it to the batch.
            </p>
          </div>
        {/if}
      </div>
      <div class="border-t border-border px-5 py-4">
        <Button
          variant="primary"
          class="w-full"
          disabled={!draftCount || submitting || !canSubmit}
          onclick={onsubmit}
        >
          {submitting
  ...
  : `Submit ${draftCount} separate billed job${draftCount === 1 ? '' : 's'}`}
        ...
        {#if !canSubmit}
          <p class="mt-2 text-center text-xs text-muted-foreground">
            Configure a Poyo API key before submitting this batch.
          </p>
        {/if}
      </div>
    </div>
