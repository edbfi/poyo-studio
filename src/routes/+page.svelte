...
<div class="route-shell">
  ...
  <section aria-labelledby="recent-heading" class="mt-8 border-t border-border pt-5">
    <div class="flex items-center justify-between gap-4"><div><p class="eyebrow-label">Recent</p><h2 id="recent-heading" class="mt-1 text-base font-semibold">Recent generations</h2></div><a class="focus-ring rounded text-xs font-semibold text-primary hover:underline" href="/gallery">Open gallery</a></div>
    {#if data.dashboard.recent.length}<div class="mt-4 grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-4">{#each data.dashboard.recent as group}<a href={`/jobs/${group.jobId}`} class="group focus-ring overflow-hidden rounded-lg border border-border bg-card no-underline"><MediaPreview mediaKind={group.representative?.mediaKind ?? group.modality} src={group.representative?.mediaUrl ?? null} alt={`Preview for ${group.displayName}`} fit="contain" class="aspect-[4/3]" /><div class="p-2.5"><p class="truncate text-xs font-semibold">{group.displayName}</p><p class="mt-0.5 truncate text-[0.6875rem] text-muted-foreground">{group.promptExcerpt ?? group.workflow}</p></div></a>{/each}</div>
