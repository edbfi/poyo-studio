  ...
  <section aria-labelledby="preset-list-heading">
    <div class="flex flex-wrap items-end justify-between gap-4 border-b border-border pb-4">
      <div>
        ...
        <h2 id="preset-list-heading" class="mt-1 text-base font-semibold tracking-tight">
          Saved presets
        </h2>
        <p class="mt-2 max-w-2xl text-sm leading-6 text-muted-foreground">
          Presets retain guided settings and input-role URLs, never large source media or
          credentials.
        </p>
      </div>
    ...
    {#if presets.length}
      <ul class="mt-5 grid list-none gap-3 p-0 md:grid-cols-2 2xl:grid-cols-3">
        {#each presets as preset (preset.id)}
          <li
            class="flex min-h-56 flex-col rounded-[var(--radius)] border border-border bg-card p-4 shadow-[var(--shadow-xs)]"
          >
            <div class="flex items-start justify-between gap-3">
              ...
              <Badge tone={preset.values.modality === 'image' ? 'info' : 'experimental'}
                >{preset.values.modality}</Badge
              >
            </div>
            ...
            <div class="mt-3 flex flex-wrap gap-1.5">
              ...
              {#if preset.values.guided.enableSafetyChecker !== undefined}
                <Badge tone="neutral"
                  >Safety {preset.values.guided.enableSafetyChecker ? 'on' : 'off'}</Badge
                >
              {/if}
              {#if preset.values.expertOverrides.length}
                <Badge tone="experimental">Expert overrides</Badge>
              {/if}
            </div>
            <div class="mt-auto flex items-center justify-between gap-3 pt-5">
              <span class="text-[0.6875rem] text-muted-foreground"
                >Updated {new Date(preset.updatedAt).toLocaleDateString()}</span
              >
              <div class="flex gap-2">
                <Button
                  variant="ghost"
                ...
                <LinkButton
                  href={`/studio/${preset.values.modality}?preset=${encodeURIComponent(preset.id)}`}
                  variant="outline"
                  >Use preset</LinkButton
                >
              </div>
            </div>
          </li>
      ...
      <div class="grid min-h-[28rem] place-items-center py-12 text-center">
        <div class="max-w-lg">
          ...
          <p class="mt-2 font-serif text-base leading-7 text-muted-foreground">
            Save a verified model configuration from either studio, then reuse it without rebuilding
            every compatible option.
          </p>
        </div>
      </div>
    {/if}
